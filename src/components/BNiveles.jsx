...
export default function BNiveles() {
  ...
  useEffect(() => {
    const fetchQuizzes = async () => {
      try {
        ...
        const biologyQuizzes = response.data.filter((quiz) => quiz.materia_id === 3); // Filtrar por biología
        ...
        {cards.map((quiz) => (
          <Grid item xs={12} sm={6} md={4} key={quiz.id}>
            <Card sx={{ display: "flex", flexDirection: "column", height: "200px" }}> {/* Ajustamos el tamaño y estilo de las tarjetas */}
              <CardContent sx={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "justify-center" }}>
                <Typography
                  variant="h6"
                  textAlign="center"
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", height: "100vh", backgroundColor: "#ecfccb" }}>
      <Box sx={{ height: "64px", backgroundColor: "#22c55e" }} />
